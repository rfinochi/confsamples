...
namespace FrasesDeTodos.Services
{
	public static class FrasesService
	{
		...
		{
			var autor = new Autor() { Nombre = "Krako", AutorId = 1 };
			...
			_frases.Add(new Frase() { Texto = "Sharepoint es como una catapulta de conejos, con la que cada tanto le pegas a tu solución", FraseId = 1, Autor = autor });
			_frases.Add(new Frase() { Texto = "Basta de peliculas de HP, el pibe ya no es un pibe! Como se va a llamar la prox?, Harry Potter y el examen de Prostata???", FraseId = 2, Autor = autor });
			_frases.Add(new Frase() { Texto = "Los hombres somos COM y las mujeres CORBA lo único que podemos intercambiar es un poco de texto plano", FraseId = 3, Autor = autor });
		}
		...
		{
			return _frases.Where(x => x.Autor.Nombre == nombreAutor).OrderBy(f => f.FraseId).ToList();
		}
