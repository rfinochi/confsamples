...
if (_om_gbls.host.indexOf("ie.microsoft.com") == -1) {
...
(function () {
	var tp = _om_gbls.path.substring(1).toLowerCase();
	//tp="testdrive/performance/demo1"; 
	var hp = "testdrive/";
	s.pageName = (tp==hp) ? tp + "default.aspx" : tp;  
	s.channel = "home";
	var pathlength = tp.split('/').length;
	if (pathlength > 1) s.channel = tp.split('/')[1];
	s.prop5 = document.title;
	var tvar="";
	// if(tp.indexOf("testdrive/demos") > -1) s.events=s.apl(s.events,"event4",",",2);
